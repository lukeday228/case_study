...
### MAKE SURE YOU HAVE THE FILE "Export5323.gpx" SAVED IN YOUR WORKING DIRECTORY.
### IT CAN BE FOUND IN THE GITHUB REPOSITORY

get_tracks<-function(data="Export5323.gpx"){
  
  gpx_parsed <- xmlParse(data)
  ...
  return(trackpoints_df)
}

trackpoints_df1 <- get_tracks(data="Export5323.gpx")
trackpoints_df2 <- get_tracks(data="gecho.gpx")
trackpoints_df3 <- get_tracks(data="AllLogsThrough3May2023.gpx")
trackpoints_df4 <- get_tracks(data="LakeWaco8June2023Track.gpx")
trackpoints_df5 <- get_tracks(data="LakeWaco9June2023Track.gpx")

# bind tracks
trackpoints_df<-rbind(trackpoints_df1,trackpoints_df2,trackpoints_df3,trackpoints_df4,trackpoints_df5)
...
lw_gpx_id$date<-as.Date(lw_gpx_id$date)
lw_water_level$date<-as.Date(lw_water_level$date)
...
ggplot() +
  coord_quickmap() +
  geom_point(size=0.75,data = lw_gpx_id_meters, shape = 19, aes(x = lon, y = lat, color = bed_elev_m)) +
  scale_colour_viridis_c(direction = 1) +
  theme_void() +
...
trackpoints_df1<-trackpoints_df
...
trackpoints_df2<-trackpoints_df


trackpoints_df <- rbind(trackpoints_df1,trackpoints_df2)
